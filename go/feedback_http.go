...
func feedbackToView(entry feedbackEntry) feedbackView {
	return feedbackView{
		ID:        entry.ID,
		Author:    entry.Author,
...
func (handler *feedbackHTTP) handleList(w http.ResponseWriter, r *http.Request) {
	...
	for _, entry := range entries {
		views = append(views, feedbackToView(entry))
	}
...
func (handler *feedbackHTTP) handleCreate(w http.ResponseWriter, r *http.Request) {
	...
	entry, err := handler.store.create(author, authorKey, input.Content)
	...
	writeJSONStatus(w, http.StatusCreated, map[string]any{"feedback": feedbackToView(entry)})
}
...
func (handler *feedbackHTTP) handleReply(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, map[string]any{"feedback": feedbackToView(entry)})
}
