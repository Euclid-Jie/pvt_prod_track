...
- 数据层以 `Euclid.fund_basic_info` 为主，追加 `Nav.PendingFund` 与 `Nav.fof99_nav_index` 作为补充来源。
- 补充产品在 `Nav.nav_interval_metrics` 中分别使用 `pending:{PROD_CODE}`、`fof99:{register_number}` 作为 `fund_code`。
