...
`loadData()` keeps `Nav.nav_interval_metrics` as a server-side pivot. Base product info comes first from `Euclid.fund_basic_info`; `Nav.PendingFund`, `Nav.AlphaPlusFund`, `Nav.fof99_nav_index`, `Nav.smw_index`, and `Nav.mail_nav_index` are appended without deduplication. Pending rows join metrics with `fund_code = 'pending:' + PROD_CODE`; Alpha Plus rows join with `fund_code = 'alpha_plus:' + PROD_CODE`; FOF99 rows join with `fund_code = 'fof99:' + register_number`; SimuWang rows join with `fund_code = 'smw:' + register_number`; enabled Mail NAV rows with non-empty `prod_comp`, `product_key`, and `prod_type` join with `fund_code = 'mail:' + product_key`; formal Euclid rows keep the existing `个人净值 -> p_{fid}` / otherwise `prod_code` rule. Pending/FOF99/SimuWang/Mail NAV scale is filled by mapping `comp_code` to `Euclid.量化私募管理人列表.登记编号`; Alpha Plus scale comes from `AlphaPlusFund.prod_comp -> Euclid.company_scale.prod_comp -> 管理规模`.
