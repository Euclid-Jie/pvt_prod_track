...
- `access.go` / `access_http.go` — Web service access approval, administrator sessions, and protected route middleware
- `feedback.go` / `feedback_http.go` — service-only public suggestions, administrator replies/deletion, and local persistence
...
Files written there: `config.json`, `access_control.json`, `feedback.json`, and `Chinese_special_holiday.txt` (if uploaded via settings). The service keeps the previous successful `feedback.json` as `feedback.json.bak` after a write.
