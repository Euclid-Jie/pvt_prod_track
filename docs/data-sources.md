...
`Nav.nav_product_preferences` 使用已经标准化的 `fund_code` 保存人工主备选择。`is_backup = 1` 的产品在周报组装阶段被排除，但净值采集和 `Nav.nav_interval_metrics` 指标计算继续保留。

该表由数据库管理员显式维护。周报只执行 `SELECT`，不得根据产品名称、净值日期或来源顺序自动推断、写入主备状态。未配置或 `is_backup = 0` 的产品均正常进入周报。
...
```sql
prod_comp IS NOT NULL AND TRIM(prod_comp) <> ''
AND register_number IS NOT NULL AND TRIM(register_number) <> ''
```
...
例子：`register_number = SAVW31` 时，对应 `Nav.nav_interval_metrics.fund_code = smw:SAVW31`。

规模补充：

`smw_index` 本身没有规模字段。展示规模通过 `comp_code` 补：
...
API 的展示字段仍格式化为两位小数，同时为近一周、近一月、YTD、近一年返回百分比单位的 `*_precise` 字段。服务版排序和平均值使用精确字段，Excel 和桌面版继续使用原展示字段。存在 `is_excess = 1` 数据的策略在 Excel 的同一策略 Sheet 内分别导出绝对收益和超额收益，超额部分沿用服务版口径排除管理人名称带“指数”的对标指数行。超额指标由上游生成，服务版和 Excel 均不得用展示值自行相减计算。
